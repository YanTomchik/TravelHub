...
async function initMap(formData, typeRender, mapActiverHotel) {
    ...
    const markers = markerDataObj.map(markerInfo => {
        ...
        if (typeRender === 'hotel-open' && id == mapActiverHotel) {
            (async () => {
                try {
        ...
        marker.addListener("click", async () => {
            ...
            const ratingBlock = rating > 0
                ...
                : '';
            const priceNetBlock = priceNet
                ? `<div class="marker-popup-footer-description"><div class="marker-popup-footer-description-main">${translationsHub?.totalNetto ?? 'Всего (нетто цена):'}</div><div class="marker-popup-footer-description-price">${priceNet} ${currencyName}</div></div>`
                : '';
            const priceStrikeBlock = priceStrike > 0
                ? `<div class="marker-popup-footer-price-alert">${priceStrike} ${markerDataCurrencyName}</div>`
                : '';
        ...
        leftBlock.addEventListener('click', event => {
            ...
            if (markerLat == curItemLat && markerLng == curItemLng) {
                map.setZoom(16);

                const offset = window.innerWidth > 1024 ? -90 : window.innerWidth >= 770 ? -130 : 0;
                centerMapZoom(offset, map, marker);
