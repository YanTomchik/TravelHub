...
async function createEventInCalendar(eventData) {
    try {
        ...
        if (response.ok) {
            ...
            const eventPageLink = dataLinks.event_page
            const googlePageLink = dataLinks.google
            const outlookPageLink = dataLinks.outlook
            const yahooPageLink = dataLinks.yahoo
            const applePageLink = dataLinks.apple
            const office365PageLink = dataLinks.office365
            const outlookcomPageLink = dataLinks.outlookcom

            const dropdownLinksContent = document.getElementById('dropdown-links-content');
            dropdownLinksContent.innerHTML = `
                            <a href="${googlePageLink}" target='blanc'>
                            ...
                            <a href="${outlookPageLink}" target='blanc'>
                                <div class="content-item-wrapper">
                                    <div class="content-icon-wrapper">
                            ...
                            <a href="${yahooPageLink}" target='blanc'>
                                <div class="content-item-wrapper">
                                    <div class="content-icon-wrapper">
                            ...
                            <a href="${applePageLink}" target='blanc'>
                                <div class="content-item-wrapper">
                                    <div class="content-icon-wrapper">
                    ...
                    `
            console.log('Событие успешно создано в календаре:', data);
        } else {
            console.error('Ошибка при создании события:', response.statusText);
        }
    } catch (error) {
...
document.addEventListener('DOMContentLoaded', function() {
    const dropdownBtn = document.getElementById("dropbtn-links-event");
    const dropdownContent = document.querySelector(".dropdown-content-wrapper");

    const tableOrders = document.querySelector('.table.table-bordered.text-center');
    const tableOrdersTd = tableOrders.getElementsByTagName('td');

    const tableDate = tableOrdersTd[0].textContent

    const numberReservation = document.getElementById('cart-accordion-rows-content').children[0].textContent.replace(/\&nbsp;/g, '');
    console.log(numberReservation)

    // Разбиваем дату на части
    let parts = tableDate.split('.');

    // Формируем новую дату в формате ISO 8601
    let isoDate = parts[2] + '-' + parts[1] + '-' + parts[0] + 'T00:00:00';
    console.log(isoDate)
    ...
    dropdownBtn.addEventListener('click', function (element) {
        ...
        const eventData = {
            ...
            description: numberReservation,
            start: isoDate,
            end: '2024-05-22T12:00:00',
            timezone: 'Europe/Moscow'
        };
    
    
        createEventInCalendar(eventData);
    });
})
...
window.addEventListener("click", function (event) {
    if (!event.target.matches('.dropdown-links-event-btn')) {
        const dropdowns = document.getElementsByClassName("dropdown-content-wrapper");
